...
コマンドキューを使用する理由は、複数のコマンドリストを使えるようにするためです。</br>
&emsp;近年のゲームは表示されるオブジェクトの数が大量になっており、描画コマンドの作成が大きなCPU側のボトルネックとなっていました。そこで、複数のスレッドで並列に描画コマンドを作成することができれば、CPUのボトルネックを大きく解消することができます。しかし、単純に描画コマンドの生成処理を並列にすればいいだけではありません。GPUの描画処理というのは処理の順番が重要になってきます。ですので、リスト2.5
